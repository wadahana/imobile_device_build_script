...
function build_ideviceinstaller() 
{
    module_name="ideviceinstaller"
    module_path="${build_path}/${module_name}"
    module_git_url="https://github.com/libimobiledevice/ideviceinstaller.git"
    echo -e "chekcout ${module_name} ..."
    if [ ! -d "$module_path" ]; then
        git clone "$module_git_url" "$module_path"  > /dev/null
    fi
    pushd "${module_path}" > /dev/null
        git reset --hard
        git clean -df
        git checkout 442f670a01f82c63d1316d60f1f47191b8bc6b5a
        ./autogen.sh --prefix="${release_path}" --enable-static > /dev/null
        patch -p1 < "${path}/ideviceinstall.patch"
        make
        make install
    popd > /dev/null
}
